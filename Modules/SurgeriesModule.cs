...
public class SurgeriesModule : ICarterModule
{
	private readonly IStorageService _storageService;
	private readonly IDatabaseService _databaseService;
	
	public SurgeriesModule(IStorageService storageService, IDatabaseService databaseService)
	{
	...
	{
		app.MapGet("/records/{patientId}/surgeries",
			async (IDocumentSession session, string patientId, CancellationToken cancellationToken) =>
			{
				...
				return record is null
					? Results.NotFound()
					: Results.Ok(record.Surgeries.IsEmpty() ? Array.Empty<MedicalHistory>() : record.Surgeries);
			});
		app.MapPost("/records/{patientId}/surgeries", async (IDocumentSession session, SurgeryDto request, string patientId, CancellationToken cancellationToken) =>
		{
			var record = await _storageService
				.GetLatestMedicalRecord(session, patientId, cancellationToken);
			if (record is null)
			{
				...
				Surgery[] surgeries = {request.ToSurgery()};
				newRecord.Surgeries = surgeries;
			...
			var prev = record.Surgeries ?? Array.Empty<Surgery>();
			var toAdd = new[] {request.ToSurgery()};
			record.Surgeries = prev
				.Concat(toAdd)
				.Distinct()
				.ToArray();
		...
		app.MapDelete("records/{patientId}/surgeries",
		async (IDocumentSession session, string patientId, [FromQuery] string prescriptionId, CancellationToken cancellationToken) =>
		{
			...
			record.Surgeries = record.Surgeries
